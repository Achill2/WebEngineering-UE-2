...
public class BigQuizServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (action.equals("Quiz starten") || action.equals("Neues Spiel")) {
			...
			QuizFactory.INSTANCE = ServletQuizFactory.init(this
					.getServletContext()); // TODO auslagern
			...
			QuizData data = (QuizData) request.getSession().getAttribute(
					"quizData");
			if (data.nextRound()) {
				// next round
				RequestDispatcher dispatcher = getServletContext()
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		long time_taken = data.getCurrentRound().getCurrentQuestion()
				.getMaxTime()
				- time_left;

		// TODO get which user ?!
		int index = data.getCurrentRound().getIndexOfCurrentQuestion();
		updateComputerAnswers(data, index);

		// set the answer status and time-value
		if (correct_answer) {
			data.getCurrentRound().getAnswersPlayer1().get(index)
		...
		// check if there is another question in this round or if the round is
		// over
		if (data.getCurrentRound().nextQuestion()) {
			...
			// TODO wirklich wieder aufrufen oder gibt es ein question.jsp
			// refresh?!
		} else {
	...
	/**
	 * check if the answer consisting of the selected choices is correct and
	 * returns true if so
	 * 
	 * @param selected_choices
	 *            - the choices selected by the user
	 * @param data
	 *            - the quizData-session object containing question and
	 *            choices-information
	 * @return true if the answer is correct, false otherwise
	 */
	private boolean checkAnswer(String[] selected_choices, QuizData data) {

		List<Choice> correct_choices = data.getCurrentRound()
				.getCurrentQuestion().getCorrectChoices();
		List<Integer> correct_ids = new ArrayList<Integer>();
		for (Choice c : correct_choices) {
			correct_ids.add(c.getId());
		}

		// (at least one answer is correct so no selected choices (null) mean
		// incorrect answer
	...
	/**
	 * Firstly generate a random number between 0 and 100, which will be used
	 * for percentage calculation if the computer-player answers a question true
	 * or false. There is 40% chance that the computer-player answers the
	 * question correctly. Then generates a number between 1 and
	 * <code>maxTime</code> for a question, which will be used for elapsed time
	 * for computer-player to answer the question. Then both values are assigned
	 * to corresponding objects.
	 * 
	 * @param data
	 *            <code>QuizData</code> for current session
	 * @param index
	 *            number of the question
	 */
	private void updateComputerAnswers(QuizData data, int index) {

		// random answer value for computer
		int cf = (int) (Math.random() * 101);
		data.getCurrentRound()
				.getAnswersPlayer2()
				.get(index)
				.setStatus(
						cf > 60 ? AnswerStatus.CORRECT : AnswerStatus.INCORRECT);

		// random used-time value for computer
		long maxTime = data.getCurrentRound().getCurrentQuestion().getMaxTime();
		long ut = (long) (Math.random() * (maxTime));
		data.getCurrentRound().getAnswersPlayer2().get(index).setTime(ut);
	}
	 ...
	 * 
	 * @param data
	 *            the QuizData of the session
	 */
	private void calculateWinnerOfRound(QuizData data) {
		List<Answer> answersPlayer1 = data.getCurrentRound()
				.getAnswersPlayer1();
		List<Answer> answersPlayer2 = data.getCurrentRound()
				.getAnswersPlayer2();
	 ...
	 * 
	 * @param data
	 *            the QuizData from current session
	 */
